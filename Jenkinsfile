	...
	stage('Build') {
		steps {
		    ...
		    sh """mkdir -p "${BUILDPATH}/Workspace"
			  ...
			  sudo rsync -av -R --exclude 'Builds' --exclude 'Jenkinsfile' --exclude 'Jenkinsfile_2' --exclude 'miniconda' --exclude 'miniconda.sh' --exclude 'README.md' --exclude 'requirements.txt' --exclude 'XmlReport' --exclude '*_test.py' --exclude '.git' --exclude '.pytest_cache' --exclude '.scannerwork' --exclude 'coverage.xml' --exclude '*.pyc' ${WORKSPACE}/  ${BUILDPATH}/Workspace/
